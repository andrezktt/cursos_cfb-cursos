...
const f_sim = () => {
    document.body.setAttribute("style", "background-color: #dfe3ee;")
}
...
btn_mensagem_1.addEventListener("click", (evento) => {
    config.tipo = "ok"
    Caixa_Mensagem.mostrar(config, "Final Fantasy XV", "Final Fantasy XV é um jogo eletrônico de RPG de ação desenvolvido e publicado pela Square Enix. Ele foi lançado mundialmente no dia 29 de novembro de 2016 para as plataformas PlayStation 4 e Xbox One.")
})
btn_mensagem_2.addEventListener("click", (evento) => {
    config.tipo = "sn"
    config.comando_sn = () => {f_sim()}
    Caixa_Mensagem.mostrar(config, "Days Gone", "Days Gone é um jogo eletrônico de ação-aventura e sobrevivência desenvolvido pela Bend Studio e publicado pela Sony Interactive Entertainment. O jogo se passa no Oregon pós-apocalíptico, dois anos após o início de uma pandemia. O ex-fora-da-lei que se tornou um vagabundo, Deacon St.")
})
btn_mensagem_3.addEventListener("click", (evento) => {
    config.tipo = "sn"
    config.textos = ["Confirmar", "Recusar"]
    config.comando_sn = () => {f_sim()}
    Caixa_Mensagem.mostrar(config, "Resident Evil Village", "Resident Evil: Village, conhecido no Japão como Biohazard: Village é um jogo eletrônico de survival horror desenvolvido e publicado pela Capcom. É a sequência de Resident Evil 7: Biohazard, de 2017, e foi anunciado pela primeira vez no evento de revelação do PlayStation 5 em 11 de junho de 2020.")
})